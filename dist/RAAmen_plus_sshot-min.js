 ...
 */(()=>{const{RAA}=window;if(RAA.isEnable){const{RPGAtsumaru}=window,{experimental}=RPGAtsumaru,{screenshot}=experimental;screenshot.displayModal=async()=>{const openModal=async src=>{await RAA.modal({message:src,decorate:s=>`<img src="${s}" />`,client:!0})};return RAA.state.screenshot.handler.then(r=>openModal(r))},screenshot.setScreenshotHandler=async promise=>{const send=await RAA.request({waitTime:RAA.responseTime.normal,post:promise,checkValid:p=>p instanceof Promise,client:!0}).then(()=>{RAA.state.screenshot.handler=promise});return send}}})();